...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const contacts = await listContactsForUser(req.auth.workspaceId, { search, filter });
...
router.delete(
  "/:contactId",
  asyncHandler(async (req, res) => {
    const result = await deleteContactForUser(req.auth.workspaceId, req.params.contactId);
    res.json(result);
  })
);
