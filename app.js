...
function initLandingHeroDemo() {
  const demoRoot = document.querySelector("[data-hero-demo]");
  if (!(demoRoot instanceof HTMLElement)) return;

  const fieldNodes = {
    name: demoRoot.querySelector('[data-hero-field="name"]'),
    email: demoRoot.querySelector('[data-hero-field="email"]'),
    brand: demoRoot.querySelector('[data-hero-field="brand"]'),
  };

  const valueNodes = {
    name: demoRoot.querySelector('[data-hero-value="name"]'),
    email: demoRoot.querySelector('[data-hero-value="email"]'),
    brand: demoRoot.querySelector('[data-hero-value="brand"]'),
  };

  const typeButtons = [...demoRoot.querySelectorAll("[data-hero-type]")];
  const publishButton = demoRoot.querySelector("[data-hero-publish]");
  ...
  function setFieldValue(key, value, showCaret = false) {
    const target = valueNodes[key];
    if (!(target instanceof HTMLElement)) return;

    target.textContent = value;
    const existingCaret = target.parentElement?.querySelector(".hero-home-v3__form-caret");
    if (existingCaret instanceof HTMLElement) {
    ...
    caret.className = "hero-home-v3__form-caret";
  ...
  function setActiveType(type, shouldHighlight = false) {
    typeButtons.forEach((button) => {
      if (!(button instanceof HTMLElement)) return;
      const isActive = button.dataset.heroType === type;
      button.classList.toggle("hero-home-v3__type-btn--active", isActive);
      button.classList.toggle("is-highlight", shouldHighlight && isActive);
    });
  }

  function setPublished(isPublished) {
    if (!(publishButton instanceof HTMLElement)) return;
    publishButton.classList.toggle("is-published", isPublished);
    publishButton.textContent = isPublished ? "Published" : "Publish booking page";
  }
  ...
  function resetDemo() {
    ...
    setFieldValue("brand", "");
    setActiveField("");
    setActiveType("strategy", false);
    setPublished(false);
  }

  function typeField(key, value, startDelay, speed) {
    value.split("").forEach((_, index) => {
      queue(() => {
        setActiveField(key);
        setFieldValue(key, value.slice(0, index + 1), true);
      }, startDelay + index * speed);
    });

    const endDelay = startDelay + value.length * speed;
    queue(() => {
  ...
  function runDemo() {
    ...
    let cursor = 260;
    cursor = typeField("name", demoValues.name, cursor, 80) + 420;
    cursor = typeField("email", demoValues.email, cursor, 72) + 460;
    cursor = typeField("brand", demoValues.brand, cursor, 78) + 620;

    queue(() => {
      setActiveField("");
      setActiveType("demo", true);
    }, cursor);

    cursor += 640;

    queue(() => {
      setActiveType("demo", false);
    }, cursor);
    ...
    restartTimeout = window.setTimeout(runDemo, cursor + 2400);
  }

  if (prefersReducedMotion) {
    setFieldValue("name", demoValues.name);
    setFieldValue("email", demoValues.email);
    setFieldValue("brand", demoValues.brand);
    setActiveField("");
    setActiveType("demo", false);
    setPublished(true);
    return;
