...
function requiredFeatureForChannel(channel) {
  const normalized = String(channel || "")
    .trim()
    .toLowerCase();
  if (normalized.includes("sms")) return "sms_reminders";
...
async function loadWorkflowRecipients(userId) {
  ...
  if (owner?.email) {
    recipients.push({
      email: owner.email,
      name: owner.display_name || owner.username || owner.email,
    });
  }
  ...
  return {
    ownerName: owner?.display_name || owner?.username || "MeetScheduling Team",
    recipients,
  };
}
...
async function updateWorkflowForUser(userId, workflowId, payload = {}) {
  ...
  await assertFeature(userId, requiredFeatureForChannel(next.channel));
...
async function runWorkflowForUser(userId, workflowId) {
  ...
  const workflow = mapWorkflowRow(result.rows[0]);
  await assertFeature(userId, requiredFeatureForChannel(workflow.channel));

  if (String(workflow.channel || "").trim().toLowerCase() === "email") {
    const { ownerName, recipients } = await loadWorkflowRecipients(userId);
    const delivery = await sendWorkflowBroadcastEmail({
      ownerName,
      workflowName: workflow.name,
      triggerLabel: workflow.trigger,
      offsetLabel: workflow.offset,
      recipients,
    });
