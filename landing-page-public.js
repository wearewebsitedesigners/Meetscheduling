(function landingPagePublicController() {
  ...
  function isCustomDomainLanding() {
    return window.location.pathname === "/" || window.location.pathname === "";
  }

  async function loadPublicPage() {
    const slug = resolveSlug();
    if (!slug && !isCustomDomainLanding()) {
      rootEl.innerHTML = `
    ...
    try {
      const endpoint = isCustomDomainLanding()
        ? "/api/public/domain/landing"
        : `/api/public/page/${encodeURIComponent(slug)}`;
