...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const payload = await listRoutingDataForUser(req.auth.workspaceId, { search, filter });
    res.json(payload);
  })
);
...
router.delete(
  "/forms/:formId",
  asyncHandler(async (req, res) => {
    const result = await deleteRoutingFormForUser(req.auth.workspaceId, req.params.formId);
    res.json(result);
  })
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const lead = await routeLeadForUser(req.auth.workspaceId, req.params.leadId, routeTo);
    res.json({ lead });
  })
);
...
router.delete(
  "/leads/:leadId",
  asyncHandler(async (req, res) => {
    const result = await deleteRoutingLeadForUser(req.auth.workspaceId, req.params.leadId);
    res.json(result);
  })
);
